...
#la URL puede ser de un sitio de grupo tambien https://xxxxxx.sharepoint.com/sites/yyyyyy
...
Write-Host "Se va a crear la lista: "$QueLista -ForegroundColor Red -BackgroundColor Yellow
    ...
    #Se añadirán después los datos desde un CSV
...
#-------------------------------------------------------------------------------
    $xml = '<Field Type="Lookup" Name="Ubicacion" DisplayName="Ubicación" 
                Description="Ubicación donde ocurre la incidencia."
                Viewable="TRUE"
	            Required="TRUE" 
...
#    Add-PnPField -List $QueLista -Type user       -DisplayName "Incidencia Asignada A"      -InternalName "IncidenciaAsignadaA"     -AddToDefaultView 
    ...
    Add-PnPFieldFromXml -FieldXml $xml -List $QueLista 
#-------------------------------------------------------------------------------
    $xml = '<Field Type="Lookup" Name="TipoIncidencia" 
        DisplayName="Tipo Incidencia" 
        Description="Tipo de incidencia. (A rellenar por el TIC)." 
        Viewable="TRUE"
        Required="FALSE" 
        List="'+$ID_IncTipos+'"
        ShowField="Title" 
...
#-------------------------------------------------------------------------------
    $xml = '<Field Type="DateTime" Name="FechaSolucion" 
        DisplayName="Fecha Solución" 
        Description="Fecha solución.  (A rellenar por el TIC)." 
        SourceID="'+$ID_IncInformaticas+'" 
...
###############################################################################
#  para ver los XML que tengo que poner puedes coger ejemplos de otro campo
#  recuperando para eso su SchemaXml
###############################################################################
#$CogerXML= Get-PnPField -List "Incidencias Informáticas" -Identity "FechaIncidencia"
#$CogerXML.SchemaXml

#$CogerXML= Get-PnPField -List "Incidencias Informáticas" -Identity "Ubicacion"
#$CogerXML.SchemaXml

#$CogerXML= Get-PnPField -List "Incidencias Informáticas" -Identity "Curso"
#$CogerXML.SchemaXml
